...
public class TransactionsInfo extends DatabaseConnection {
    ...
    public TransactionsInfo(Context context) {
    ...
    public Transaction detailOfTransaction(String transactionID){
        ...
        try {
            try (Cursor cursor = database.rawQuery("select * from " + tableName + " where " + TransactionsInfo.transactionId + " = '" + transactionID + "';", null)) {
                cursor.moveToFirst();
    ...
    public static long count(Context context) {
        long count = 0;
        try {
            TransactionsInfo customers = new TransactionsInfo(context);
